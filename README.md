Nöfn nemenda í verkefninu:
Ramon de la Rosa, Jóhann Karl Hirst, Snædís Perla Sigurðardóttir, Ísabella Montazeri

Útlit:

Almennur litur á letri er `#000000` og grunnstærð leturs er `16px`.

í haus skal vera notaður bakgrunnslitur `#afb281` . Fyrirsögnin með leturstærð `4em` og linkarnir með leturstærð ``. linkar í haus skulu vera undirstrikaðir ef við erum stödd á þeirri síðu og letrið breytir um lit við hover og fer í `#ffffff`

Í fót skal nota bakgrunnslit `#cc9694`  . 
Litur á letri í linkum skal vera `#fcffd2` og breytist í `#000000` við hover
Fyrirsagnirnar skulu vera í `""` og linkarnir í `""`. Fyrir Copyright skal nota `""`.

fyrir buttons skal nota `#cc9694` og `#000000` þegar það er hoverað yfir button.
litur á letri í button breytist í `#cc9694` við hover (fyrir utan "uppfæra" og "eyða úr körfu" buttons í cart og products, fyrir þá gildir `#ffffff` sem breytist í `#cc9694` við hover)

Meginmál á forsíðu:
bakgrunnslitur `#ffffff`
leturstæðr á fyrirsögnum skulu vera `""`
og á texta `""`.

Meginmál starfsfólk:
leturstærð `""`.

Meginmál cart og products:
leturstærð á fyrirsögnum `""` og texta `""`.
Verð skal vera í `""` og bold. 
...
Slóð verkefnis:
https://github.com/sps13/Hopverkefni1.git
öllum breytingum skal vera push-að inn á þetta verkefni.
